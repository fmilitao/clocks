...
type ClockMaker = new (x: number, y: number, s: Snap.Paper) => Clock;

interface Clock {

    // no animation
    set(hour: number, minutes: number, seconds: number): void;

    // with animation
    tick(hour: number, minutes: number, seconds: number): void;

};

const ARC_60 = 360 / 60, ARC_12 = 360 / 12;
const SUB_ARC_60 = ARC_60 / 60, SUB_ARC_12 = ARC_12 / 60;
const ANIMATION_TIME = 300;
const TICK = 1000; // 1 second, updates clock every second


function main(clockMaker: ClockMaker) {

    const s = Snap(window.innerWidth, window.innerHeight);

    const x = Math.floor(window.innerWidth / 2);
    const y = Math.floor(window.innerHeight / 2);

    // creates clock with center (x,y)
    const clock = new clockMaker(x, y, s);
...
function bounce(transformation: string, element: Snap.Element) {
    element.animate({ 'transform': transformation }, ANIMATION_TIME, mina.bounce);
};

function linear(transformation: string, element: Snap.Element) {
    element.animate({ 'transform': transformation }, ANIMATION_TIME, mina.linear);
};

function toAngles(h: number, m: number, s: number): [number, number, number] {
    return [
...
function toStrictAngles(h: number, m: number, s: number): [number, number, number] {
...
class ClockWork {
    ...
    protected hours: Snap.Element;
    protected minutes: Snap.Element;
    protected seconds: Snap.Element;

    protected rotateS: (angle: number) => string;
    protected rotateM: (angle: number) => string;
    protected rotateH: (angle: number) => string;

    protected oldhours: number;
    protected oldminutes: number;
    protected oldseconds: number;

    constructor(x: number, y: number, s: Snap.Paper) {
        this.oldhours = 0;
        this.oldminutes = 0;
        this.oldseconds = 0;

        // all use the same rotation transformation
        this.rotateS = function(angle: number) {
            return 'r' + angle + ',' + x + ',' + y;
        };
    ...
    adjust(oldAngle: number, newAngle: number,
        element: Snap.Element,
        transformation : (n:number) => string ) {
    ...
    set(h: number, m: number, s: number) {
        [this.oldhours, this.oldminutes, this.oldseconds] = this.angler()(h, m, s);
...
class SimpleClock extends ClockWork implements Clock {
    ...
    constructor(x: number, y: number, s: Snap.Paper) {
        ...
        const FONT_H = 30;
        const MSG = 'Just write the goddamn paper.';
        ...
        // text
        const text = s.text(x, y + 3 * FONT_H, MSG);
        text.attr({
...
class HolesClock extends ClockWork implements Clock {
    ...
    constructor(x: number, y: number, s: Snap.Paper) {
        ...
        const FONT_H = 30;
        const PADDING = 10;
        const POS = RADIUS - FONT_H - PADDING;
        const ARC = (2 * Math.PI * ARC_12) / 360;
        const POS2 = RADIUS + FONT_H + PADDING * 2;
        const ARC2 = (2 * Math.PI * ARC_60) / 360;
        const FONT_H2 = 15;

        const pattern = s.image("images/wood.jpg", 0, 0, 640, 300).toPattern(0, 0, 300, 200);
        const pattern2 = s.image("images/wood2.png", 0, 0, 500, 500).toPattern(0, 0, 500, 500);
        ...
        for (let i = 0; i < 12; ++i) {
            const angle = -i * ARC;
            const xx = x + POS * Math.cos(angle);
            const yy = y - POS * Math.sin(angle);
            const text = s.text(xx, yy, ((i + 2) % 12 + 1) + '');
            text.attr({
                fontFamily: 'Lobster',
                fontSize: FONT_H,
                textAnchor: 'middle',
        ...
        for (let i = 0; i < 60; ++i) {
            const angle = -i * ARC2;
            const xx = x + POS2 * Math.cos(angle);
            const yy = y - POS2 * Math.sin(angle);
            const text = s.text(xx, yy, ((i + 15) % 60) + '');
            text.attr({
                fontFamily: 'Lobster',
                fontSize: FONT_H2,
                textAnchor: 'middle',
        ...
        const disk = s.circle(x, y, RADIUS).attr({
            fill: pattern, //'rgba(200,0,0,0.9)',
        ...
        const disk2 = s.circle(x, y, POS2 + PADDING * 3 + FONT_H2).attr({
            fill: pattern, //'rgba(200,0,0,0.9)',
            stroke: 'rgba(0,0,0,0.5)',
            strokeWidth: 22 // wtf? why doesn't 4 look the same??
        });

        const group2 = s.group();
        group2.append(
            s.circle(x, y, POS2 + PADDING * 2 + FONT_H2).attr({
                fill: 'white',
                stroke: 'white'
...
class HandsClock extends ClockWork implements Clock {
    ...
    constructor(x: number, y: number, s: Snap.Paper) {
        ...
        const PADDING = 10;
        const POS = RADIUS - FONT_H - PADDING;
        const ARC = (2 * Math.PI * ARC_12) / 360; // to radians
